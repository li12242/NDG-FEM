...
for n = 1:Ndeg
    for m = 1:Nmesh
        dbd = DamBreakDryUniformMesh2d( order(n), ne(m), type, 0 );
        tic; dbd.matSolve(); time(m, n) = toc;
        fext = dbd.GetExactFunction( 20 );
        
        tmp = dbd.meshUnion.evaluateNormErr2( dbd.fphys{1}, fext{1} );
        err2(m, n) = tmp(1);
%         tmp = dbd.meshUnion.evaluateNormErr1( dbd.fphys{1}, fext{1} );
%         err1(m, n) = tmp(1);
%         tmp = dbd.meshUnion.evaluateNormErrInf( dbd.fphys{1}, fext{1} );
%         errInf(m, n) = tmp(1);
