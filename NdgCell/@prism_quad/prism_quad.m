classdef prism_quad < ndg_lib.std_cell.std_cell
    %PRISM_QUAD Summary of this class goes here
    %   Detailed explanation goes here
    
    properties(Constant)
        type = ndg_lib.std_cell_type.Prism_Quad % 单元类型
        Nv = 8
        vol = 8
        vr = [-1,  1,  1, -1, -1,  1,  1, -1]'
        vs = [-1, -1,  1,  1, -1, -1,  1,  1]'
        ...
        Nface = 6
        Nfv = [4, 4, 4, 4, 4, 4];
        FToV = [1,2,3,4; 1,2,6,5; 2,3,7,6; 3,4,8,7; 4,1,5,8; 5,6,7,8]';
        faceType = [...
    ...
    methods
        function obj = prism_quad(N)
            obj = obj@ndg_lib.std_cell.std_cell(N);
        end% func
        
        function [ node_val ] = project_vert2node(vert_val)
            node_val = 0.25*(...
                (1-obj.r).*(1-obj.s)*vert_val(1,:) + ...
                (1+obj.r).*(1-obj.s)*vert_val(2,:) + ...
                (1+obj.r).*(1+obj.s)*vert_val(3,:) + ...
                (1-obj.r).*(1+obj.s)*vert_val(4,:) ).*(1-obj.t)*0.5 + ...
                0.25*(...
                (1-obj.r).*(1-obj.s)*vert_val(5,:) + ...
                (1+obj.r).*(1-obj.s)*vert_val(6,:) + ...
                (1+obj.r).*(1+obj.s)*vert_val(7,:) + ...
                (1-obj.r).*(1+obj.s)*vert_val(8,:) ).*(1+obj.t)*0.5;
        end
        
        function draw(obj, varargin)
            figure('color', 'w');
            subplot(1, 2, 1); % 标准单元顶点分布
            v1 = [1, 2, 3, 4, 1, 2, 3, 4, 5, 6, 7, 8];
            v2 = [2, 3, 4, 1, 5, 6, 7, 8, 6, 7, 8, 5];
            g = graph(v1, v2);
