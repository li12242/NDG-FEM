...
%> @details This function computes the N^2 nodes and weights for a triangle
%> with vertices given by the 3x2 vector v. The nodes are produced by
%> collapsing the square to a triangle.
%>
%> Sample Usage:
%> @code
%> [X,Y,Wx,Wy] = ndgcell.triquad(8,[0 0; 0 2; 2 0])
%> f = @(x,y) exp(x+y); Q = Wx'*f(X,Y)*Wy;
%> @endcode
%>
%> Reference: J.N. Lyness, Ronald Cools, A Survey of Numerical Cubature
%>            over Triangles (1994)
%> Author:    Greg von Winckel, gregvw(at)math(dot)unm(dot)edu
