...
void mexFunction(int nlhs, mxArray *plhs[],
	...
	real *xc    = (real*) malloc(sizeof(real)*K );
	...
	for (k=0;k<K;k++){
		...
		for(i=0;i<Nfaces;i++){
			...
			// int v1 = (int) Fmask[i] - 1; // local vertex indics
			...
			if( e != k ){
				...
				real yb = (y[sk2] + y[sk1])/2.0;

				real d1 = (xb - xc[k])*(xb - xc[k]) + (yb - yc[k])*(yb - yc[k]);
				real d2 = (xb - xc[e])*(xb - xc[e]) + (yb - yc[e])*(yb - yc[e]);
