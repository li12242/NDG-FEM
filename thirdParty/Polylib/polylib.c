...
void zwgj(double *z, double *w, int np, double alpha, double beta) {
  ...
  fac =
      pow(two, apb + one) * gammaF(alpha + np + one) * gammaF(beta + np + one);
...
void Dgrjm(double *D, double *Dt, double *z, int np, double alpha,
           double beta) {
  if (np <= 0) {
    D[0] = Dt[0] = 0.0;
  } else {
    ...
    for (i = 0; i < np; i++)
      for (j = 0; j < np; j++) {
        if (i != j)
          ...
          if (i == 0)
            D[i * np + j] =
                -(np + alpha + beta + one) * (np - one) / (two * (beta + two));
          else
...
void Dgrjp(double *D, double *Dt, double *z, int np, double alpha,
           double beta) {
  if (np <= 0) {
    D[0] = Dt[0] = 0.0;
  } else {
    ...
    for (i = 0; i < np; i++)
      for (j = 0; j < np; j++) {
        if (i != j)
          ...
          if (i == np - 1)
            D[i * np + j] =
                (np + alpha + beta + one) * (np - one) / (two * (alpha + two));
          else
            D[i * np + j] = (alpha - beta - one + (alpha + beta + one) * z[i]) /
                            (two * (one - z[i] * z[i]));
        }
    ...
    for (i = 0; i < np; i++)
      for (j = 0; j < np; j++) {
        if (i != j)
          ...
          if (i == 0)
            D[i * np + j] =
                (alpha - (np - 1) * (np + alpha + beta)) / (two * (beta + two));
          else if (i == np - 1)
            D[i * np + j] = -(beta - (np - 1) * (np + alpha + beta)) /
                            (two * (alpha + two));
          else
...
void Imgj(double *im, double *zgj, double *zm, int nz, int mz, double alpha,
          double beta) {
  double zp;
  register int i, j;
  ...
  for (i = 0; i < mz; ++i) {
    zp = zm[i];
    for (j = 0; j < nz; ++j) im[i * nz + j] = hgj(j, zp, zgj, nz, alpha, beta);
  }
...
void Imgrjm(double *im, double *zgrj, double *zm, int nz, int mz, double alpha,
            double beta) {
  double zp;
  register int i, j;
...
void Imgrjp(double *im, double *zgrj, double *zm, int nz, int mz, double alpha,
            double beta) {
  double zp;
  register int i, j;
...
void Imglj(double *im, double *zglj, double *zm, int nz, int mz, double alpha,
           double beta) {
  double zp;
  register int i, j;
...
void jacobfd(int np, double *z, double *poly_in, double *polyd, int n,
  ...
  if (n == 0) {
    if (poly_in)
      for (i = 0; i < np; ++i) poly_in[i] = one;
    if (polyd)
      for (i = 0; i < np; ++i) polyd[i] = zero;
  } else if (n == 1) {
    if (poly_in)
      for (i = 0; i < np; ++i)
        poly_in[i] = 0.5 * (alpha - beta + (alpha + beta + two) * z[i]);
    if (polyd)
      for (i = 0; i < np; ++i) polyd[i] = 0.5 * (alpha + beta + two);
  } else {
    register int k;
    double a1, a2, a3, a4;
...
/*
  ...
  \author li12242
*/
void jacobiP(int np, double *z, double *poly_in, int n, double alpha,
             double beta) {
  double r;
  int i;
  jacobfd(np, z, poly_in, NULL, n, alpha, beta);
  r = (2 * n + alpha + beta + 1) / pow(2, alpha + beta + 1) *
      gammaF(n + alpha + beta + 1) * factorial(n) / gammaF(n + alpha + 1) /
      gammaF(n + beta + 1);
  for (i = 0; i < np; i++) {
...
void GradjacobiP(int np, double *z, double *dp, int n, double alpha,
                 double beta) {
  int i;
  double r;
  if (n < 1) {
...
void jacobd(int np, double *z, double *polyd, int n, double alpha,
            double beta) {
  register int i;
  double one = 1.0;
  if (n == 0)
...
static void JacZeros(int n, double *a, double alpha, double beta) {
  ...
  b[n - 1] = pow(2.0, apb + 1.0) * gammaF(alpha + 1.0) * gammaF(beta + 1.0) /
             gammaF(apbi);
  a[0] = (beta - alpha) / apbi;
  b[0] =
      sqrt(4.0 * (1.0 + alpha) * (1.0 + beta) / ((apbi + 1.0) * apbi * apbi));

  a2b2 = beta * beta - alpha * alpha;
  for (i = 1; i < n - 1; ++i) {
    apbi = 2.0 * (i + 1) + apb;
    a[i] = a2b2 / ((apbi - 2.0) * apbi);
    b[i] = sqrt(4.0 * (i + 1) * (i + 1 + alpha) * (i + 1 + beta) *
