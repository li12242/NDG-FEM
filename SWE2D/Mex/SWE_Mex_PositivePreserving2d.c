 ...
 * 
 * Usages:
 *		[hp, qxp, qyp] = SWE_Mex_PositivePreserving2d(h, qx, qy, M, J, hmin);
 */
void mexFunction(int nlhs, mxArray *plhs[], 
	...
	real *qy   = mxGetPr(prhs[2]);
	real *M    = mxGetPr(prhs[3]);
	real *J    = mxGetPr(prhs[4]);
	real hcri  = mxGetScalar(prhs[5]);
	...
	for (i=0;i<K;i++){
		...
		for(j=0;j<Np;j++){
		    ...
		    qyp[ind] = theta*(qy[ind] - qymean) + qymean;
