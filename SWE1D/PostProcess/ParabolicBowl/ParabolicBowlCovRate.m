...
%         plot(x, h-he, '-b.');
    ...
    rateQ = calConvRate(errorH, nele);
    
    fprintf('========================= n = %d =========================\n',order(ideg));
    fprintf('----------------------- rate of h -----------------------\n');
    printResult(rateH, nele, errorType);
    fprintf('----------------------- rate of q -----------------------\n');
    printResult(rateQ, nele, errorType);
    fprintf('\n');
end% for

end% func

function printResult(rate, nele, errorType)
ne = numel(nele);
switch errorType
    case 1
        fprintf('nele, \t L2\n');
