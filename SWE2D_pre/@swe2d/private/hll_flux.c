 ...
 */
void hll_flux(double hmin, double gra, double hM, double hP, double qnM,
              double qnP, double qvM, double qvP, double *Fhn, double *Fqxn,
              double *Fqyn) {
 ...
 *  h, qx, qy, z  - conservative variables;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  signed char *eidtype = (signed char *)mxGetData(prhs[13]); // int8 类�?
...
#pragma omp parallel for private(j) num_threads(DG_THREADS)
  ...
  for (i = 0; i < K; i++) {
    ...
    for (j = 0; j < Nfp; j++) {
      ...
      qnM = f_M[1] * nx_ + f_M[2] * ny_;
      qvM = -f_M[1] * ny_ + f_M[2] * nx_;
      qnP = f_P[1] * nx_ + f_P[2] * ny_;
      qvP = -f_P[1] * ny_ + f_P[2] * nx_;
      // qnM =  qxM*nx_ + qyM*ny_; qvM = -qxM*ny_ + qyM*nx_;
      // qnP =  qxP*nx_ + qyP*ny_; qvP = -qxP*ny_ + qyP*nx_;

      double Fhns, Fqns, Fqyns;
      hll_flux(hmin, gra, f_M[0], f_P[0], qnM, qnP, qvM, qvP, &Fhns, &Fqns,
               &Fqyns);
      ...
      nodal_flux(hmin, gra, f_M[0], f_M[1], f_M[2], z[iM], &Eh, &Eqx, &Eqy, &Gh,
                 &Gqx, &Gqy);
